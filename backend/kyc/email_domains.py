"""Disposable / temporary email domain filtering.

Registration is the entry point to a KYC pipeline, so accounts must be tied
to a real, lasting inbox. Disposable ("burner") email services let someone
receive the signup OTP and then vanish, defeating both identity verification
and any future account-recovery or audit contact.

The check is a curated blocklist of well-known disposable providers. It is
intentionally self-contained — no external API or network call — so it is
fast, deterministic, and cannot be bypassed by a third-party outage. The
list is a snapshot of the most common providers; extend ``DISPOSABLE_DOMAINS``
as new services appear.

The server is the source of truth: the SPA mirrors this rule in
``frontend/src/validation.ts`` for immediate feedback, but a direct API
client is still stopped here.
"""

from __future__ import annotations

# Known disposable / temporary email providers. Lowercase, bare domains
# (no scheme, no subdomain prefix). Matching is done on the exact domain
# part of the address, case-insensitively.
DISPOSABLE_DOMAINS: frozenset[str] = frozenset(
    {
        "10minutemail.com",
        "10minutemail.net",
        "10minutemail.org",
        "20minutemail.com",
        "anonbox.net",
        "anonymbox.com",
        "burnermail.io",
        "dispostable.com",
        "emailondeck.com",
        "fakeinbox.com",
        "getairmail.com",
        "getnada.com",
        "guerrillamail.com",
        "guerrillamail.net",
        "guerrillamail.org",
        "guerrillamailblock.com",
        "inboxkitten.com",
        "mailinator.com",
        "mailnesia.com",
        "maildrop.cc",
        "mailcatch.com",
        "mail-temp.com",
        "mintemail.com",
        "mohmal.com",
        "mytemp.email",
        "nada.email",
        "sharklasers.com",
        "spamgourmet.com",
        "spam4.me",
        "temp-mail.org",
        "tempail.com",
        "tempinbox.com",
        "tempmail.de",
        "tempmailer.com",
        "tempr.email",
        "throwawaymail.com",
        "trashmail.com",
        "trashmail.net",
        "trash-mail.com",
        "yopmail.com",
        "yopmail.net",
        "yopmail.fr",
        "mail-temporaire.fr",
        "jetable.org",
        "courriel.fr.nf",
        "moncourrier.fr.nf",
        "spambox.us",
        "mt2015.com",
        "mt2014.com",
        "my10minutemail.com",
        "tempomail.fr",
        "disposableemailaddresses.com",
        "mailnull.com",
        "spamhole.com",
        "sneakemail.com",
        "gishpuppy.com",
        "mailmetrash.com",
        "thankyou2010.com",
        "trashymail.com",
        "mailexpire.com",
        "tempinbox.co.uk",
        "filzmail.com",
        "mailmoat.com",
        "mytrashmail.com",
        "trashemail.de",
        "spamfree24.org",
        "mailshell.com",
        "dodgeit.com",
        "haltospam.com",
        "spamex.com",
        "pookmail.com",
        "lazyinbox.com",
        "tempmailer.de",
        "wegwerfmail.de",
        "wegwerfmail.net",
        "wegwerfmail.org",
        "binkmail.com",
        "safetymail.info",
        "temp-mail.io",
        "tempmailaddress.com",
        "emailtemporanea.com",
        "emailtemporanea.net",
        "tempmailo.com",
        "1secmail.com",
        "1secmail.net",
        "1secmail.org",
        "esiix.com",
        "wwjmp.com",
        "xmaily.com",
        "mailto.plus",
        "mailpooch.com",
        "tmpmail.net",
        "tmpmail.org",
        "boun.cr",
        "fexbox.org",
        "fexpost.com",
        "fextemp.com",
        "any.pink",
        "merepost.com",
        "placebomail10.com",
        "postheaven.fun",
        "rover.info",
        "incognitomail.org",
        "mailforspam.com",
        "spamdecoy.net",
        "tempmail.plus",
        "disposablemail.com",
        "emailfake.com",
        "generator.email",
        "guerrillamail.info",
        "mailgutter.com",
        "mailhub.pro",
        "mailprotech.com",
        "moakt.co",
        "moakt.ws",
        "mvrht.com",
        "mvrht.net",
        "nospam.ze.tc",
        "spam.la",
        "spamtrap.ro",
        "tradermail.info",
        "veryrealemail.com",
        "33mail.com",
        "maildrop.ml",
        "maildrop.ga",
        "maildrop.gq",
        "maildrop.cf",
        "guerrillamail.biz",
        "guerrillamail.de",
        "grr.la",
        "guerrillamailblock.org",
        "pokemail.net",
        "spamobox.com",
        "tempomail.org",
        "mailinator.net",
        "mailinator.org",
        "mailinator2.com",
        "notmailinator.com",
        "sogetthis.com",
        "streetwisemail.com",
        "suremail.info",
        "thisisnotmyrealemail.com",
        "tradermail.info",
        "veryrealmail.com",
        "mailmetrash.com",
        "deadaddress.com",
        "despammed.com",
        "emailias.com",
        "emailex.com",
        "front14.org",
        "ghosttexter.de",
        "jetable.net",
        "kasmail.com",
        "link2mail.net",
        "maileater.com",
        "mailexpire.com",
        "mailfreeonline.com",
        "mailnator.com",
        "mailslite.com",
        "mailzilla.com",
        "mbx.cc",
        "meltmail.com",
        "nervmich.net",
        "nervtmich.net",
        "netmails.com",
        "netmails.net",
        "niepodam.pl",
        "nospamfor.us",
        "objectmail.com",
        "obobbo.com",
        "oneoffemail.com",
        "onewaymail.com",
        "oopi.org",
        "ordinaryamerican.net",
        "otherinbox.com",
        "ourklips.com",
        "outlawspam.com",
        "ovpn.to",
        "owlpic.com",
        "pancakemail.com",
        "pimpedupmyspace.com",
        "pjjkp.com",
        "politikerclub.de",
        "poofy.org",
        "pookmail.com",
        "privacy.net",
        "proxymail.eu",
        "prtnx.com",
        "putthisinyourspamdatabase.com",
        "quickinbox.com",
        "rcpt.at",
        "recode.me",
        "recursor.net",
        "regbypass.com",
        "rejectmail.com",
        "rklips.com",
        "rmqkr.net",
        "royal.net",
        "rtrtr.com",
        "s0ny.net",
        "safe-mail.net",
        "safersignup.de",
        "safetypost.de",
        "sandelf.de",
        "saynotospams.com",
        "selfdestructingmail.com",
        "sendfree.org",
        "shiftmail.com",
        "shitmail.me",
        "shitmail.org",
        "shitware.nl",
        "shortmail.net",
        "sibmail.com",
        "sinnlos-mail.de",
        "siteposter.net",
        "slaskpost.se",
        "slushmail.com",
        "smellfear.com",
        "snakemail.com",
        "sneakemail.com",
        "sofimail.com",
        "sofort-mail.de",
        "sogetthis.com",
        "soodonims.com",
        "spam.la",
        "spamail.de",
        "spamarrest.com",
        "spambob.com",
        "spambob.net",
        "spambob.org",
        "spambog.com",
        "spambog.de",
        "spambog.ru",
        "spambox.info",
        "spambox.org",
        "spamcannon.com",
        "spamcannon.net",
        "spamcon.org",
        "spamcorptastic.com",
        "spamcowboy.com",
        "spamcowboy.net",
        "spamcowboy.org",
        "spamday.com",
        "spamgourmet.com",
        "spamgourmet.net",
        "spamgourmet.org",
        "spamherelots.com",
        "spamhereplease.com",
        "spamhole.com",
        "spamify.com",
        "spaminator.de",
        "spamkill.info",
        "spaml.com",
        "spaml.de",
        "spammotel.com",
        "spamobox.com",
        "spamoff.de",
        "spamslicer.com",
        "spamspot.com",
        "spamthis.co.uk",
        "spamthisplease.com",
        "spamtrail.com",
        "spamtroll.net",
        "speed.1s.fr",
        "spoofmail.de",
        "stuffmail.de",
        "super-auswahl.de",
        "supergreatmail.com",
        "supermailer.jp",
        "superrito.com",
        "superstachel.de",
        "suremail.info",
        "talkinator.com",
        "teewars.org",
        "teleworm.com",
        "teleworm.us",
        "temp-mail.ru",
        "temp15qm.com",
        "tempalias.com",
        "tempe-mail.com",
        "tempemail.biz",
        "tempemail.co.za",
        "tempemail.com",
        "tempemail.net",
        "tempinbox.com",
        "tempmail.eu",
        "tempmail.it",
        "tempmail2.com",
        "tempmaildemo.com",
        "tempmailer.com",
        "tempmailer.de",
        "tempomail.fr",
        "temporarily.de",
        "temporarioemail.com.br",
        "temporaryemail.net",
        "temporaryemail.us",
        "temporaryforwarding.com",
        "temporaryinbox.com",
        "thanksnospam.info",
        "thankyou2010.com",
        "thc.st",
        "thelimestones.com",
        "thisisnotmyrealemail.com",
        "thismail.net",
        "throwawayemailaddress.com",
        "tilien.com",
        "tittbit.in",
        "tizi.com",
        "tmailinator.com",
        "toomail.biz",
        "topranklist.de",
        "tradermail.info",
        "trash-mail.at",
        "trash-mail.com",
        "trash-mail.de",
        "trash2009.com",
        "trashdevil.com",
        "trashdevil.de",
        "trashemail.de",
        "trashmail.at",
        "trashmail.com",
        "trashmail.de",
        "trashmail.me",
        "trashmail.net",
        "trashmail.org",
        "trashmail.ws",
        "trashmailer.com",
        "trashymail.com",
        "trashymail.net",
        "trbvm.com",
        "trialmail.de",
        "trillianpro.com",
        "tryalert.com",
        "turual.com",
        "twinmail.de",
        "tyldd.com",
        "uggsrock.com",
        "umail.net",
        "upliftnow.com",
        "uplipht.com",
        "uroid.com",
        "us.af",
        "venompen.com",
        "veryrealemail.com",
        "viditag.com",
        "viewcastmedia.com",
        "viewcastmedia.net",
        "viewcastmedia.org",
        "webm4il.info",
        "webuser.in",
        "wegwerfadresse.de",
        "wegwerfemail.com",
        "wegwerfemail.de",
        "wegwerfmail.de",
        "wegwerfmail.info",
        "wegwerfmail.net",
        "wegwerfmail.org",
        "wetrainbayarea.com",
        "wetrainbayarea.org",
        "wh4f.org",
        "whyspam.me",
        "willhackforfood.biz",
        "willselfdestruct.com",
        "winemaven.info",
        "wronghead.com",
        "wuzup.net",
        "wuzupmail.net",
        "wwwnew.eu",
        "xagloo.com",
        "xemaps.com",
        "xents.com",
        "xmaily.com",
        "xoxy.net",
        "yapped.net",
        "yeah.net",
        "yep.it",
        "yogamaven.com",
        "yopmail.com",
        "yopmail.fr",
        "yopmail.net",
        "yourdomain.com",
        "ypmail.webarnak.fr.eu.org",
        "yuurok.com",
        "zehnminuten.de",
        "zehnminutenmail.de",
        "zippymail.info",
        "zoemail.net",
        "zoemail.org",
        "zomg.info",
    }
)


def is_disposable_email(email: str) -> bool:
    """Return True when the address's domain is a known disposable provider.

    Only the domain part is inspected (case-insensitively). The local part
    is irrelevant to the check. Malformed input (no ``@``) returns False —
    basic format validation is handled elsewhere, so this stays focused.
    """
    if "@" not in email:
        return False
    domain = email.rsplit("@", 1)[1].strip().lower()
    return domain in DISPOSABLE_DOMAINS
